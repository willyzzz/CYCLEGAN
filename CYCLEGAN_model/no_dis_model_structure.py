...
class Generator_sc_to_bulk(nn.Module):
    ...
    def forward(self, sig, fra):
        fake_bulk = torch.matmul(fra, sig)  # (batch_size, gene_dim)
        fake_bulk = self.bulk_denoise(fake_bulk)  # apply Conv1d layers
        return fake_bulk
...
#             nn.ELU(),
#
#             nn.Conv1d(32, cell_type_dim, kernel_size=1),
#             nn.AdaptiveAvgPool1d(1), # 将 gene_dim 维度缩减为 1
#             nn.Softplus()
#         )
#
...
#         bulk = bulk.unsqueeze(1)  # (batch_size, 1, gene_dim)
#         fake_fra = self.model(bulk)  # Apply Conv1d layers
#         fake_fra = fake_fra.squeeze(2)  # (batch_size, cell_type_dim)

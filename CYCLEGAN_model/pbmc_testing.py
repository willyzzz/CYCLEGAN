...
from model_structure_0728 import *
...
train_bulk_path = '../pbmc_data/sc_exp/sc_exp_donorA.txt'
train_fra_gt_path = '../pbmc_data/sc_fra/donorA_gt.txt'
...
os.makedirs(os.path.join(fra_save_path, testing_dataset_name), exist_ok=True)
...
dataloader, bulk, sig, fra_gt, fra_ini = pbmc_dataset_process(data_path_list, mode='test', log_icon=True)
...
D_sc = Discriminator_sc(cell_type_dim, gene_dim, device).to(device)
...
optimizer_G_sc_bulk = optim.RMSprop(G_sc_bulk.parameters(), lr=1e-4)
optimizer_G_bulk_sc = optim.RMSprop(G_bulk_sc.parameters(), lr=1e-4)
optimizer_D_bulk = optim.RMSprop(D_bulk.parameters(), lr=learning_rate, weight_decay=1e-2)
optimizer_D_sc = optim.RMSprop(D_sc.parameters(), lr=learning_rate, weight_decay=1e-2)
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(dataloader):
        ...
        set_requires_grad([D_bulk, D_sc], False)
        ...
        pred_fake_fra = D_sc(fake_fra)

        loss_GAN_bulk = criterion_GAN(pred_fake_bulk, torch.ones_like(pred_fake_bulk))
        loss_GAN_fra = criterion_GAN(pred_fake_fra, torch.ones_like(pred_fake_fra))
        ...
        # 计算最终的生成器损失
        loss_G = lamda_GAN * (loss_GAN_bulk + loss_GAN_fra) + lambda_cycle * (loss_cycle_bulk + loss_cycle_fra)
        loss_G.backward()
        optimizer_G_sc_bulk.step()
        optimizer_G_bulk_sc.step()

        # 计算判别器损失
        set_requires_grad([D_bulk, D_sc], True)
        optimizer_D_bulk.zero_grad()
        optimizer_D_sc.zero_grad()
        ...
        pred_real_fra = D_sc(real_fra_data)
        loss_D_fra_real = criterion_GAN(pred_real_fra, torch.ones_like(pred_real_fra))

        pred_fake_fra = D_sc(fake_fra.detach())
        loss_D_fra_fake = criterion_GAN(pred_fake_fra, torch.zeros_like(pred_fake_fra))

        loss_D_fra = (loss_D_fra_real + loss_D_fra_fake) * 0.5

        # 总判别器损失
        loss_D = loss_D_bulk + 0.5 * loss_D_fra
        loss_D.backward()
        optimizer_D_sc.step()
    ...
    # 计算当前epoch的ccc值
    current_ccc = ccc(fake_G1_full_fra_cat, fra_gt, mode='avg')
...
# 画出ccc曲线
plt.plot(range(num_epochs), ccc_values)
plt.xlabel('Epoch')
plt.ylabel('CCC')
plt.title('CCC Curve')
plt.show()
...
df = pd.DataFrame(best_fake_G1_full_fra_cat.cpu().numpy())
df.to_csv(os.path.join(fra_save_path, testing_dataset_name,'pre.txt'), sep='\t')
...
with torch.no_grad():
    ...
    print('Test fra ccc: ', ccc(fake_fra_test, fra_gt, mode='avg'))
    # 在这里可以添加其他评估指标的计算和打印

...
sig_path = '../pbmc_data/sc_sig/or_donorA_sig.txt'
train_bulk_path = '../pbmc_data/sc_exp/sc_exp_donorC.txt'
train_fra_gt_path = '../pbmc_data/sc_fra/donorC_gt.txt'
...
dataloader, bulk, sig, fra_gt, fra_ini = pbmc_dataset_process(data_path_list, mode='test', log_icon=True, minmax_icon=True)
...
optimizer_D_bulk = optim.RMSprop(D_bulk.parameters(), lr=1e-5, weight_decay=1e-3)
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(dataloader):
        ...
        # pred_real_bulk = D_bulk(real_bulk_data)
        # pred_fake_bulk = D_bulk(fake_bulk.detach())
        # loss_D_bulk = -torch.mean(pred_real_bulk) + torch.mean(pred_fake_bulk)
        #
        # clip_value = 0.1
        # for p in D_bulk.parameters():
